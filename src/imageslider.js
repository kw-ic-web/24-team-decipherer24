...
const ImageSlider = () => {
  ...
  useEffect(() => {
    if (location.state?.solved) {
      ...
      markStageComplete(startIndex); // 올바른 스테이지 완료로 업데이트
    }
  }, [location.state]);
  ...
  const nextImage = () => {
    if (currentIndex < 2) {
      if (stageCompleted[currentIndex] || (location.state?.solved && currentIndex === 2)) {
  ...
  const handleCenterClick = (e) => {
    ...
    if (x >= x1 && x <= x2 && y >= y1 && y <= y2) {
      const stageStates = {
        0: { state: { stageIndex: 0 } },
        1: { state: { stageIndex: 1 } },
        2: { state: { stageIndex: 2 } },
      };
  
      navigate(`/game/stage${currentIndex + 1}`, stageStates[currentIndex]);
    }
  };  
